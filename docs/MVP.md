...
- 正式设置页从 sub2api `GET /v1/models` 加载、搜索和选择模型，提供刷新；鉴权失败、空列表和网络错误分别显示，手填 ID 仅为备用入口。
- 显式支持 Chat Completions 与 Responses；连接测试和 Pi 实际执行使用同一协议。两种协议分别验证流式输出、工具参数与结果往返、取消、错误和 usage，W0 单协议结果不能替代双协议验收。
- 获取并展示模型元信息及来源；实施前核对 sub2api 实际版本提供的字段/接口。标准模型列表不承诺完整能力与价格，缺失信息显示未知，不以硬编码的模型名猜测能力，不将未知价格/用量变为零。
- W1 负责精简配置界面与模型发现，W3 负责 Pi Worker 双协议实际执行；在两者完成前保持验收项未通过。
