...
状态：2026-09-20 grill-with-docs 访谈完成；Q1—Q13 及交互规则已整体确认。下一阶段为可操作原型；尚未形成 Spec、拆票或实现。
...
| Q4 快捷操作 | 大部分操作优先填入聊天框，由用户发送；预览批注及审阅改稿路径待核查 Craft 实现后进一步确认。 |
...
下一阶段为可操作原型；原型验收完成后再整理 Spec、拆分任务并实现。
