    ...
    void (async () => {
      ...
      try {
        ...
        if (run.status === 'save_failed') {
          ...
          if (await optionalFileInfo(sandbox, '/tmp/agentanywhere-session/checkpoint.jsonl')) await saveCheckpoint(run, sandbox)
          if (run.pending_status === 'succeeded' || await optionalFileInfo(sandbox, `${outputDir}/manifest.json`)) await persistArtifacts(run, sandbox)
        }
