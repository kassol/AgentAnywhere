...
export async function createWorkStore(databaseUrl: string) {
  ...
  async function detail(id: string) {
    const [row] = await db`SELECT t.id, t.goal, t.source_url AS "sourceUrl", t.status, t.created_at AS "createdAt",
      ...
      r.previous_report_version_id AS "previousReportVersionId", h.id AS "threadId"
  ...
  return { list, detail, events, create, continueTask, appendRunMessage, pendingRunMessages, acknowledgeRunMessage, artifactVersion, requestCleanupRetry, resolveRunModelConnection, authorizeModelProxy, recordModelUsage }
}
