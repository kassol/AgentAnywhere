/*
 * Adapted from packages/ui/src/components/overlay/AnnotatableMarkdownDocument.tsx in Craft Agents OSS v0.13.3 at
 * e8963854c3679edcceb105a42537a06749e6cb64.
 * Copyright 2026 Craft Docs Ltd. Licensed under Apache-2.0.
 * Local changes: inject the safe Markdown renderer and browser persistence
 * callbacks; remove Craft block shift-click; restore Version drafts against
 * exact DOM ranges; preserve persisted annotation identity across draft-clear
 * retries; use the browser-preview portal host to escape the A panel stack.
 */
import * as React from 'react'
import type { AnnotationV1 } from './annotations/types'
import { type IslandTransitionConfig } from './Island'
...
import { persistSelectionWithDraftCleanup } from './annotations/selection-persistence'
...
export interface AnnotatableMarkdownDocumentProps {
  ...
  onAddAnnotation?: (messageId: string, annotation: AnnotationV1) => boolean | void
  onRemoveAnnotation?: (messageId: string, annotationId: string) => boolean | void
  onUpdateAnnotation?: (messageId: string, annotationId: string, patch: Partial<AnnotationV1>) => boolean | void
  onDraftChange?: (selection: TextAnnotationSelection, note: string, persistedAnnotationId?: string) => boolean | void
  onDraftClear?: () => boolean | void
  restoreDraft?: { selection: TextAnnotationSelection; note: string; persistedAnnotationId?: string } | null
  openSelectionRequest?: { selection: TextAnnotationSelection; note: string; nonce: number } | null
...
export function AnnotatableMarkdownDocument({
  ...
  const persistedSelectionAnnotationIdRef = React.useRef<string | null>(null)

  const closeSelectionMenu = React.useCallback(() => {
    if (pendingSelection) onDraftClear?.()
    closeAll()
  }, [closeAll, onDraftClear, pendingSelection])
  ...
  React.useEffect(() => {
    const requestedDraft = openSelectionRequest ?? restoreDraft
    if (!requestedDraft || !canAnnotate) return
    const key = openSelectionRequest
      ? `${messageId}:request:${openSelectionRequest.nonce}`
      : `${messageId}:restore:${requestedDraft.selection.start}:${requestedDraft.selection.end}:${requestedDraft.note}`
    if (restoredDraftKeyRef.current === key) return
    const root = contentLayerRef.current
    if (!root) return
    const fullText = getCanonicalText(root)
    const selection = requestedDraft.selection
    if (fullText.slice(selection.start, selection.end) !== selection.selectedText) return
    const range = resolveRangeFromOffsets(root, selection.start, selection.end)
    const rects = range ? Array.from(range.getClientRects()).filter(rect => rect.width > 0 && rect.height > 0) : []
    const rect = rects[0] ?? range?.getBoundingClientRect() ?? root.getBoundingClientRect()
    restoredDraftKeyRef.current = key
    persistedSelectionAnnotationIdRef.current = 'persistedAnnotationId' in requestedDraft
      ? requestedDraft.persistedAnnotationId ?? null
      : null
    openFromSelection({
      ...selection,
    ...
    openFollowUpFromSelection()
    setFollowUpDraft(requestedDraft.note)
    if (openSelectionRequest) onDraftChange?.(selection, requestedDraft.note)
    selectionMenuOpenedAtRef.current = Date.now()
  }, [canAnnotate, messageId, onDraftChange, openFollowUpFromSelection, openFromSelection, openSelectionRequest, restoreDraft,
    selectionMenuOpenedAtRef, setFollowUpDraft])
  ...
  const handleSubmitFollowUp = React.useCallback((note: string) => {
    ...
    if (activeAnnotationDetail) {
      ...
      const accepted = onUpdateAnnotation(messageId, activeAnnotationDetail.annotationId, {
        body: nextBody,
    ...
    const result = persistSelectionWithDraftCleanup({
      ...
      persistedAnnotationId: persistedSelectionAnnotationIdRef.current,
      persist: annotationId => onAddAnnotation(messageId, { ...annotation, id: annotationId }),
      rememberPersistedDraft: annotationId => onDraftChange?.(pendingSelection, normalizedNote, annotationId),
      clearDraft: () => onDraftClear?.(),
    })
    persistedSelectionAnnotationIdRef.current = result.persistedAnnotationId
    if (result.status !== 'complete') return
    markSubmitSuccess()
    clearDomSelection()
  }, [activeAnnotationDetail, onUpdateAnnotation, activeAnnotation, closeSelectionMenu, pendingSelection, canAnnotate, onAddAnnotation, messageId, sessionId, markSubmitSuccess, onDraftChange, onDraftClear])

  const cancelAndRestore = useAnnotationCancelRestore({
    contentRootRef: contentLayerRef,
    cancelFollowUp,
  })
  const handleCancelFollowUp = React.useCallback(() => {
    const hadPendingSelection = Boolean(pendingSelection)
    cancelAndRestore()
    if (hadPendingSelection) onDraftClear?.()
  }, [cancelAndRestore, onDraftClear, pendingSelection])

  const handleDeleteActiveAnnotation = React.useCallback(() => {
    if (!onRemoveAnnotation || !activeAnnotationDetail) return
    const accepted = onRemoveAnnotation(messageId, activeAnnotationDetail.annotationId)
    if (accepted === false) return
    markDeleteSuccess()
  ...
  const showSelectionMenuFromCurrentSelection = React.useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      openFromSelection(nextSelection)
      onDraftChange?.(nextSelection, '')
      selectionMenuOpenedAtRef.current = Date.now()
      dragStartPointerRef.current = null
    })
  }, [annotations, canAnnotate, closeSelectionMenu, triggerSelectionMenuEntryReplay, onDraftChange])
  ...
  const selectionMenu = (
    <AnnotationIslandMenu
      ...
      onDraftChange={next => {
        ...
        if (pendingSelection) onDraftChange?.(pendingSelection, next, persistedSelectionAnnotationIdRef.current ?? undefined)
      }}
      ...
      zIndex={islandZIndex}
      usePortal={shouldRenderAnnotationIslandInPortal('browser-preview')}
    />
  )
