/*
 * Adapted from packages/ui/src/components/markdown/annotation-resolver.ts in Craft Agents OSS v0.13.3 at
 * e8963854c3679edcceb105a42537a06749e6cb64.
 * Copyright 2026 Craft Docs Ltd. Licensed under Apache-2.0.
 * Local changes: use the local AnnotationV1 type; require position/quote agreement and reject ambiguous repeated quote matches.
 */
...
function findQuoteRange(
  ...
  let exactMatch: { start: number; end: number } | null = null
  ...
  while (searchIndex !== -1) {
    ...
    if (prefixOk && suffixOk) {
      ...
      exactMatch = { start: candidateStart, end: candidateEnd }
    }
  ...
  let normalizedMatch: { start: number; end: number } | null = null
  ...
  while (normalizedIndex !== -1) {
    ...
    if (prefixOk && suffixOk) {
      ...
      if (originalStart != null && originalLast != null) {
        ...
        normalizedMatch = { start: originalStart, end: originalLast + 1 }
      }
    }
...
export function resolveTextAnnotations(
  ...
  for (const annotation of annotations) {
    ...
    if (
      ...
      (!quote?.exact || fullText.slice(position.start, position.end) === quote.exact)
    ) {
      resolved.push({
        annotation,
