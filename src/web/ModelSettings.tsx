...
type State = { endpoint: string; hasCredential: boolean; catalog: { id: string; name: string; ownedBy?: string }[]; models: Model[]; defaultModel: string | null; discovery: { status: string; updatedAt?: string } }
const failure: Record<string, string> = { unauthorized: '网关拒绝凭证（401）。请检查密钥。', timeout: '模型列表请求超时，请检查网关。', empty: '网关返回空模型列表。已有选择已保留。', error: '模型列表加载失败，请检查端点和网关。' }
...
export function ModelSettings() {
  ...
  async function send(path: string, method: string, body?: unknown) {
    ...
    if (result.endpoint !== undefined) setState(result)
  ...
  return <section className="settings-card model-settings" aria-labelledby="model-title">
    ...
    {state && <>
      ...
      {state.models.map(model => <div className="selected-model" key={model.id}>
        ...
        {(!model.contextWindow || !model.maxTokens || !model.input || model.reasoning === undefined) && <p className="error">缺少 Pi 必要运行参数，请补充上下文、输出上限、输入模态与推理能力。</p>}
