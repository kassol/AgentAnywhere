...
export function ModelSettings() {
  ...
  async function saveModels() {
    ...
    setBusy(true); setMessage('')
    try {
      await send('/api/model-connection/models', 'PUT', modelSelectionPayload(state))
  ...
  return <section className="settings-card model-settings" aria-labelledby="model-title">
    <header className="model-settings-header">
      <div><h2 id="model-title">模型设置</h2><p>先设置日常使用的管家模型和人工调研模型池。连接、协议与能力依据收纳在详情中。</p></div>
      {dirtyModels && <span className="model-unsaved">有未保存更改</span>}
    </header>
    {!state && !message && <p className="model-loading" role="status">正在读取模型配置…</p>}
    <fieldset className="settings-controls" disabled={busy}>
      {message && <p className="model-message" role="status">{message}</p>}
      {state && failure[state.discovery.status] && <p role="alert" className="error model-message">上次网关刷新：{failure[state.discovery.status]} 已保留上次有效模型列表与选择。</p>}
      {state && <>
        <section className="model-section model-daily" aria-labelledby="model-daily-title">
          <div className="model-section-heading"><div><h3 id="model-daily-title">日常使用</h3><p>保存只影响之后的新管家轮次和工作；进行中的模型快照保持不变。</p></div></div>
          <div className="model-group-card">
            <div className="model-setting-row"><div><label htmlFor="steward-model">管家模型</label><p>新管家轮次使用的模型。</p></div><select id="steward-model" value={state.stewardModel?.modelId ?? ''} onChange={event => { setDirtyModels(true); setState({ ...state, stewardModel: event.target.value ? { modelId: event.target.value, protocol: state.stewardModel?.protocol ?? 'chat-completions' } : null }) }}><option value="">未设置</option>{state.models.map(model => <option key={model.id} value={model.id}>{model.id}</option>)}</select></div>
            <div className="model-setting-row"><div><label htmlFor="steward-protocol">管家协议</label><p>按模型连接实际支持的协议选择。</p></div><select id="steward-protocol" value={state.stewardModel?.protocol ?? 'chat-completions'} disabled={!state.stewardModel} onChange={event => { setDirtyModels(true); setState({ ...state, stewardModel: state.stewardModel && { ...state.stewardModel, protocol: event.target.value as Protocol } }) }}><option value="chat-completions">Chat Completions</option><option value="responses">Responses</option></select></div>
          </div>
          <div className="model-section-heading model-pool-heading"><div><h3>人工调研模型池</h3><p>只有勾选并保存的模型可供管家选择。网关发现列表不会自动入池。</p></div><span>{state.researchModelPool.length} 个已选</span></div>
          ...
          })}</div> : <p className="error">尚无已选模型，请在连接与模型详情中添加模型并补全运行参数。</p>}
          ...
          <div className="model-save-row"><button type="button" onClick={saveModels} disabled={busy || !dirtyModels}>保存日常设置</button></div>
        </section>

        <details className="model-details">
          <summary><span>连接与模型详情</span><small>网关、凭证、模型发现、默认协议、能力来源和人工覆盖参数</small></summary>
          <div className="model-details-body">
            <section className="model-section" aria-labelledby="model-connection-title">
              <div className="model-section-heading"><div><h3 id="model-connection-title">模型连接</h3><p>配置一套 sub2api 网关。凭证只写入服务端，页面不会回显。</p></div></div>
              <form className="model-form model-group-card" onSubmit={saveConnection}>
            ...
            <section className="model-section" aria-labelledby="model-discovery-title">
              <div className="model-section-heading"><div><h3 id="model-discovery-title">发现与参考目录</h3><p>网关列表记录可见模型，参考目录补充公开能力信息。</p></div></div>
              <div className="model-group-card model-refresh-card">
                <div className="model-setting-row"><div><strong>网关模型列表</strong><p>{state.discovery.updatedAt ? `上次刷新：${new Date(state.discovery.updatedAt).toLocaleString()}` : '尚未刷新'}</p></div><button type="button" className="secondary" onClick={() => refresh('/api/model-connection/refresh')} disabled={busy || !state.hasCredential}>刷新</button></div>
                <div className="model-setting-row"><div><strong>参考目录</strong><p>{state.directory.successAt ? `缓存：${new Date(state.directory.successAt).toLocaleString()} · ${state.directory.cachedModels} 项` : '尚无缓存'}{state.directory.status && state.directory.status !== 'ok' && state.directory.status !== 'never' ? ` · 本次${failure[state.directory.status] ?? state.directory.status}` : ''}</p></div><button type="button" className="secondary" onClick={() => refresh('/api/model-connection/directory')}>刷新</button></div>
              </div>
              {state.catalogSourceEndpoint && <p className="muted model-source">目录来源：{state.catalogSourceEndpoint}</p>}
              <p className="muted model-source">列出模型不代表协议、工具或推理能力已验证。</p>
              <label className="search-label">搜索网关模型<input type="search" value={search} onChange={event => setSearch(event.target.value)} /></label>
            ...
            <section className="model-section" aria-labelledby="selected-models-title">
              <div className="model-section-heading"><div><h3 id="selected-models-title">已选模型</h3><p>逐项核对协议、目录映射、能力来源和人工覆盖值。</p></div><span>{state.models.length} 个模型</span></div>
              <label className="default-model model-group-card">默认工作模型<select value={state.defaultModel ?? ''} onChange={event => { setDirtyModels(true); setState({ ...state, defaultModel: event.target.value || null }) }}><option value="">未设置</option>{state.models.map(model => <option key={model.id} value={model.id}>{model.id}</option>)}</select></label>
              <div className="selected-models">{state.models.map(model => <details className="selected-model" key={model.id}>
                ...
                <div className="selected-model-body">
                  <div className="model-heading"><strong>运行与能力详情</strong><button type="button" className="secondary" onClick={() => removeModel(model.id)}>移除模型</button></div>
                  ...
                  <button type="button" className="secondary model-test-button" onClick={() => testModel(model)} disabled={busy || dirtyModels || !state.hasCredential || endpoint !== state.endpoint || apiKey.length > 0}>测试已保存连接与协议</button>
              ...
              <div className="model-save-row"><button type="button" onClick={saveModels} disabled={busy || !dirtyModels}>保存全部模型设置</button></div>
            </section>
          </div>
        </details>
      </>}
