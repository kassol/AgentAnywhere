...
type ResumeAction = '调研' | '追加' | '取消' | '回答' | '重试' | '改稿'
type Props = {
  turnId: string
  ...
  onResume: (operationId: string, action: ResumeAction) => void
}
...
function Resume({ operationId, status, action, enabled, onResume }: { operationId: string; status: string; action: ResumeAction; enabled: boolean; onResume: Props['onResume'] }) {
  if (!enabled || !['accepted', 'unexecuted'].includes(status)) return null
  const command = `继续${action}回执 ${operationId}`
  return <button type="button" className="secondary" aria-label={command} onClick={() => onResume(operationId, action)}>填入继续命令</button>
}

export function StewardReceipts({ turnId, research, controls, interactions, retries, revisions, onResume }: Props) {
  ...
  return <div className="turn-receipts">
    {ownResearch.map(operation => {
      ...
      return <article className="turn-receipt" key={operation.operationId}><strong>调研派发 · {label[operation.status] ?? operation.status}</strong>
        ...
        <Resume {...{ operationId: operation.operationId, status: operation.status, action: '调研', enabled: true, onResume }} />
      </article>
    })}
    {ownControls.map(operation => <article className="turn-receipt" key={operation.operationId}><strong>{operation.kind === 'steer' ? '追加要求' : '取消工作'} · {controlStatus(operation)}</strong>
      ...
      <Resume {...{ operationId: operation.operationId, status: operation.status, action: operation.kind === 'steer' ? '追加' : '取消', enabled: Boolean(operation.taskId && operation.runId), onResume }} />
    </article>)}
    {ownInteractions.map(operation => <article className="turn-receipt" key={operation.operationId}><strong>{operation.interactionKind === 'limit' ? '额度决定' : '回答问题'} · {label[operation.status] ?? operation.status}</strong>
      ...
      <Resume {...{ operationId: operation.operationId, status: operation.status, action: '回答', enabled: Boolean(operation.taskId && operation.runId && operation.interactionId && operation.epoch !== undefined), onResume }} />
    </article>)}
    {ownRetries.map(operation => <article className="turn-receipt" key={operation.operationId}><strong>{operation.mode === 'same' ? '同模型重试' : '替代模型重试'} · {label[operation.status] ?? operation.status}</strong>
      ...
      <Resume {...{ operationId: operation.operationId, status: operation.status, action: '重试', enabled: Boolean(operation.taskId && operation.sourceRunId && operation.modelId), onResume }} />
    </article>)}
    {ownRevisions.map(operation => <article className="turn-receipt" key={operation.operationId}><strong>报告改稿 · {label[operation.status] ?? operation.status}</strong>
      ...
      <Resume {...{ operationId: operation.operationId, status: operation.status, action: '改稿', enabled: Boolean(operation.taskId && operation.sourceVersionId && operation.modelId), onResume }} />
    </article>)}
  </div>
}
