...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from './craft/components/Select'
...
export function ModelSelect({ value, models, onValueChange }: { value: string; models: Model[]; onValueChange(value: string): void }) {
  return <Select value={value} onValueChange={next => {
    if (models.some(model => model.id === next)) onValueChange(next)
  }}>
    <SelectTrigger aria-label="模型"><SelectValue placeholder="请选择模型" /></SelectTrigger>
    <SelectContent>{models.map(model => <SelectItem key={model.id} value={model.id}>{model.id}</SelectItem>)}</SelectContent>
  </Select>
}

function ProtocolSelect({ value, onValueChange }: { value: string; onValueChange(value: string): void }) {
  return <Select value={value} onValueChange={onValueChange}>
    <SelectTrigger aria-label="协议"><SelectValue /></SelectTrigger>
    <SelectContent>
      <SelectItem value="default">使用模型默认协议</SelectItem>
...
export function Work() {
  ...
  if (detailId) return <div className="work-detail-page">
    <Button asChild variant="ghost" size="sm" className="work-back"><a href="/tasks"><ArrowLeft aria-hidden="true" />返回工作列表</a></Button>
    ...
    {detail && <section className="work-detail">
      ...
      {detail.runs.length > 1 && <details className="work-history"><summary>执行历史 · {detail.runs.length} 次</summary><ul>{detail.runs.map(run => <li key={run.id}>Run {run.id.slice(0, 8)} · {workStatusLabel(run.status)}{run.retryOfRunId && ` · 重试 ${run.retryOfRunId.slice(0, 8)}`}</li>)}</ul></details>}
      ...
      {detail.interaction?.status === 'pending' && detail.interaction.kind === 'question' && <form onSubmit={submitAnswer}>
        <PermissionRequest title="需要回答" identity={`Task ${detail.id} · Run ${detail.run.id} · Interaction ${detail.interaction.id}`}
          description={detail.interaction.question} actions={<Button size="sm" disabled={busy || !answer.trim() || detail.run.status !== 'waiting' || detail.run.cleanupState !== 'cleaned'} type="submit">提交回答</Button>}>
          <div className="grid gap-2"><Label htmlFor="work-answer">回答</Label><Textarea id="work-answer" value={answer} onChange={event => setAnswer(event.target.value)} maxLength={4000} required rows={3} /></div>
        </PermissionRequest>
      </form>}
      {detail.interaction?.status === 'pending' && detail.interaction.kind === 'limit' && <PermissionRequest title="需要决定"
        identity={`Task ${detail.id} · Run ${detail.run.id} · Interaction ${detail.interaction.id}`} description={detail.interaction.question}
        actions={<><Button size="sm" type="button" disabled={busy || detail.run.cleanupState !== 'cleaned'} onClick={() => void decideLimit('continue')}>增加额度并继续</Button>
          <Button size="sm" variant="ghost" type="button" disabled={busy || detail.run.cleanupState !== 'cleaned'} onClick={() => void decideLimit('finish')}>结束执行</Button></>} />}
      {detail.interaction?.status === 'answered' && <p className="work-message">回答：{detail.interaction.answer}</p>}
      {['failed', 'lost'].includes(detail.run.status) && detail.run.cleanupState === 'cleaned' && <Button className="work-standalone-action" variant="outline" size="sm" type="button" disabled={busy} onClick={() => void retry()}><RotateCcw aria-hidden="true" />手动重试（创建新 Run）</Button>}
      {detail.thread.messages.map((message, index) => <div className="work-message-entry" key={index}><UserMessageBubble content={message.content} compactMode />
        {message.status === 'pending' && <small className="muted">待处理{detail.run.status === 'running' ? '，将在下一模型步骤生效' : '，本次执行已结束，等待继续处理'}</small>}
      ...
      {(detail.run.cleanupState === 'failed' || detail.run.cleanupState === 'blocked') && <div className="error work-cleanup-error" role="alert"><span>{detail.run.cleanupState === 'blocked' ? '成果尚未安全保存；沙箱已保留。' : '沙箱回收失败，请重试。'}</span><Button variant="outline" size="sm" type="button" onClick={retryCleanup}>重试保存与回收</Button></div>}
      ...
      {detail.artifacts.length > 0 && <section className="artifacts"><header><h3>成果</h3>{currentVersion && <a href={`/api/artifacts/${currentVersion}/download`}>下载 Markdown</a>}</header>
        <div className="artifact-versions" role="group" aria-label="报告版本">{detail.artifacts.filter(item => item.kind === 'report').map((item, index, reports) => <Button variant="outline" size="sm" type="button" key={item.versionId} onClick={() => setSelectedVersion(item.versionId)} aria-pressed={currentVersion === item.versionId}>第 {reports.length - index} 版<span>{new Date(item.createdAt).toLocaleString('zh-CN')} · Run {item.runId.slice(0, 8)}{item.runStatus !== 'succeeded' && ' · 未完成'}</span></Button>)}</div>
        {currentVersion && <>
