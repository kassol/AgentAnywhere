...
2026-09-18 从专用网络内的临时容器查询“`SearXNG 开源 搜索 引擎`”：JSON HTTP 200、40 条结果，本次 `unresponsive_engines` 为空。结果提供标题、URL、摘要；`publishedDate` 可以为 `null`。这次成功不保证每次查询所有引擎都可用，调用方须保留部分结果与失败引擎信息。
