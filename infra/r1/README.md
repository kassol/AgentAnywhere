...
真实 sub2api 的双协议连接测试和 Pi 工具往返分别见 [R1-05](../../docs/evidence/r1-05.md) 与 [R1-06](../../docs/evidence/r1-06.md)。`submit_report` 在沙箱内写报告与文本附件，queue 校验后提交 Artifact/Version。执行中追加要求通过 `POST /api/runs/:id/messages` 保存，并由当前 epoch 的 Pi 在下一模型步骤接收；隔离流式回归运行 `node infra/r1/test-steering.mjs`，结果见 [R1-09](../../docs/evidence/r1-09.md)。取消和正式浏览器接管由后续票验收。

R1-08 的 queue 同时连接 `agentanywhere-r1-search` 专用网络，从固定 `http://agentanywhere-r1-searxng:8080` 查询 JSON；沙箱仅凭本次 Run 短 token 调用 queue 的 `search_web` 与 `open_public_page` 工具入口。queue 从 `service.env` 读取 `AGENTANYWHERE_PUBLIC_ORIGIN` 以拒绝控制面 URL。公开网页逐跳校验 DNS 解析地址并固定连接目标，仅提取 HTTP 文本；搜索结果包含部分失败的引擎。测试环境将 `SEARCH_ORIGIN` 指向同网络的模型/搜索 HTTP fixture；`node infra/r1/test-research.mjs` 从公开 API 验证主题和指定 URL 的报告，并通过 `/waiting-search`、`/release-search` 控制真实工具执行中的搜索响应。该脚本需要 `TEST_PASSWORD_FILE` 和固定的 19112/19113 隔离端口。
