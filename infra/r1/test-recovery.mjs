...
const interruptedEvents = await api(`/api/tasks/${interrupted.id}/events`)
assert.equal(interruptedEvents.filter(event => event.type === 'tool.started' && event.payload.name === 'echo_observation').length, 1)
...
const recoveredEvents = await api(`/api/tasks/${interrupted.id}/events`)
assert.equal(recoveredEvents.filter(event => event.type === 'tool.started' && event.payload.name === 'echo_observation').length, 0)
...
const finishedLimit = await create('fixture-limit')
const finishWaiting = await until(finishedLimit.id, 'waiting')
assert.equal(finishWaiting.interaction.kind, 'limit')
assert.equal(finishWaiting.run.modelCalls, 40)
await api(`/api/interactions/${finishWaiting.interaction.id}/resolve`, 'POST', { answer: 'finish' }, 202)
const finished = await until(finishedLimit.id, 'cancelled')
assert.equal(finished.run.modelCallLimit, 40)
assert.equal(finished.run.epoch, 1)
const finishDecisions = (await api(`/api/tasks/${finishedLimit.id}/events`)).filter(event => event.type === 'run.limit_decided')
assert.equal(finishDecisions.length, 1)
assert.equal(finishDecisions[0].payload.decision, 'finish')
noSandbox(finishedLimit.run.id)
...
console.log(JSON.stringify({ transient: 'retried-without-tool-replay', failure: 'checkpoint-retried', interrupted: 'echo-once-then-manual-retry', modelLimit: '40-then-explicit-continue-or-finish', sandboxes: 'none-active' }))
