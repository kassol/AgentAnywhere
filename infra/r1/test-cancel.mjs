...
execFileSync('docker', ['exec', 'agentanywhere-r1-test-queue-1', 'node', '--input-type=module', '-e', checkSandboxes, ...[streamed, queued, responseRun, partial, complete].map(task => task.run.id)], { stdio: 'pipe' })
console.log(JSON.stringify({ queued: 'cancelled', chat: 'cancelled', responses: 'cancelled', savedReport: true, completedRace: 'succeeded', sandboxes: 'none-active' }))
