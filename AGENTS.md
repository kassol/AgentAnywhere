...
自托管的个人委托工作台。已实现独立 Web 登录、模型连接、工作创建、两种协议的最小隔离执行与持久事件、报告成果保存和阅读，以及执行中追加要求的持久化和流式续跑。完成后继续工作已通过本地公开 API 验证，完整隔离链路待验；搜索与其他交互能力仍在实施中。产品要求见 [PRD](docs/PRD.md)，实施范围见 [MVP](docs/MVP.md)。
