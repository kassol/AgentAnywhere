...
自托管的个人委托工作台。已实现独立 Web 登录、模型连接、工作创建、两种协议的最小隔离执行与持久事件、报告成果保存和阅读、执行中追加要求及受控搜索工具。完成后继续工作、取消、提问和回答恢复已接入；完整隔离验收仍待完成。产品要求见 [PRD](docs/PRD.md)，实施范围见 [MVP](docs/MVP.md)。
